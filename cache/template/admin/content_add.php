        ...
        <table width="100%" border="0" cellpadding="8" cellspacing="0" class="tableBasic">
            <tbody>
                ...
                <tr>
                    ...
                    <td><input type="text" name="url" value="" size="80" class="inpMain"> 链接到其他内容时填写</td>
                </tr>
                <tr>
                ...
                <tr id="addonRow" class="hide">
                    ...
                    <td>
                        <div id="addon"></div>
                        <div><div id="previewbox"></div><button type="button" class="btn">上传</button></div>
                    </td>
                </tr>
                <tr>
                    <td align="right"></td>
...
$('#addonRow button').on('click', function(){
    ...
    var upParams = {
        ...
        init:{
            FileUploaded: function(uploader, fileinfo){
                url = buckets[bucket] + '/' + fileinfo.target_name;
            },
    ...
    var params = {
        ...
        onOk:function(){
            ...
            var txt = $.trim($('input[name=brief]', this).val());
            ...
            if(bucket == 'image'){
                ...
                listitem = createImagePreview(idx, url, txt);
            }

            if(bucket == 'video'){

                listitem = createVideoPreview(idx, url);
            }

            if(listitem){

                $(listitem).appendTo($('#previewbox'));

                data[idx] = {idx: {type: bucket, url: url, txt: txt}};
            }
        }
    };
...
$('#previewbox').delegate('.imagebox', 'mouseenter', function(){

    $('.mask', this).fadeIn('fast');
})

$('#previewbox').delegate('.imagebox', 'mouseleave', function(){

    $('.mask', this).fadeOut('fast');
})

$('#previewbox').delegate('.del', 'click', function(){
...
$('#previewbox').delegate('.edit', 'click', function(){
    ...
    var params = {
        onCreate:function(){
            ...
            $('input[name=brief]').val(item.txt);
        },
        onOk:function(){

            data[idx].txt = $('input[name=brief]').val();
        }
    };

    pWin.showConfirm($('#popContainer').html(), '编辑' + data[idx].type == 'image' ? '图片' : '视频', params);
});

function createImagePreview(idx, imageUrl, imageDesc){

    return $('<div class="imagebox" data-id="'+ idx +'"><div class="imagecontainer"><img src="' + imageUrl + '" width="100%" height="100%"/></div><div class="imagetext" title="' + imageDesc + '">' + imageDesc + '</div><div class="mask hide"><a class="edit" data-id="' + idx + '" href="javascript:;">编辑</a> <a class="del" data-id="' + idx + '" href="javascript:;">删除</a></div></div>');
}

function createVideoPreview(idx, videoUrl){

    return $('<div class="videobox" data-id="'+ idx +'"></div>');
}
